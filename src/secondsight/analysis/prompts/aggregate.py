...
Renders the SD §5.5.3 Step-2 LLM prompt via the jinja2 template at
`prompts/analysis/aggregate.jinja2` (Task 3 refactor).
...
ONE call per flag_type that clusters semantically-similar flag
occurrences and produces convention text for each cluster.
...
- The convention spec ("≤ 200 tokens, 2-5 sentences, actionable") is
  rendered verbatim into the prompt's task block.
- AggregatePattern.occurrence_count is constrained to ≥ 1.
"""
...
def build_aggregate_prompt(flag_type: BehaviorFlagType, flags: list[FlagSummary]) -> str:
    """Render the SD §5.5.3 Step-2 prompt for ONE flag_type group via jinja2.
    ...
    still produces a syntactically valid prompt.

    Raises:
        TypeError: if flag_type is not a BehaviorFlagType instance.
        jinja2.UndefinedError: if a required template variable is absent
            (DC9 protection from StrictUndefined).
    """

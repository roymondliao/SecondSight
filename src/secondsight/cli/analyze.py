...
def _build_orchestrator(
    *,
    ...
    """Build the Orchestrator with all its dependencies.

    Uses select_model → LLMRouter → PydanticAIAnalysisAgent chain.
    Falls back gracefully if model selection fails (logs WARNING).

    Assumption: global config lives at secondsight_home/config.toml;
    ...
    # Type narrowing for the injected db_engine
    _db: _DBEngine = db_engine  # type: ignore[assignment]

    events_repo = EventsRepository(_db)
    runs_repo = AnalysisRunsRepository(_db)
    flags_repo = BehaviorFlagsRepository(_db)
    directives_repo = DirectivesRepository(_db)
    reports_repo = SessionReportsRepository(_db)

    for repo in [flags_repo, directives_repo, reports_repo]:
        repo.create_schema()

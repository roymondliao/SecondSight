...
> **Source.** Sections 1–10 are copied verbatim from
> [`awesome-design-md`](https://github.com/VoltAgent/awesome-design-md) ·
> `design-md/claude/DESIGN.md` (MIT). The **"SecondSight Overrides"** section
> at the bottom is project-specific delta — read both before generating UI.
...
## SecondSight Overrides

> The sections above describe **Claude's marketing surface** — editorial,
> magazine-paced, anchored by serif display type and 96px section rhythm.
> SecondSight is a **local-only dashboard** for observation / analysis /
> directives. The overrides below state where SecondSight deliberately
> diverges from Claude, and why. Read these *after* the Claude tokens so the
> base intent is in head first.
>
> **Inheritance rule:** if a SecondSight rule below contradicts the Claude
> section above, the SecondSight rule wins. If a token isn't listed here, the
> Claude default holds.

### Product context (read first)
- **Audience:** developers running AI coding agents (Claude Code, OpenCode, Codex) locally. Single-project MVP, ~5 s polling dashboard.
- **Reading posture:** *contemplative review*, not real-time ops. Users come to SecondSight to inspect what their agent did, mark behavior flags, edit directive lifecycle — slow, dense, deliberate.
- **Underlying ethos:** Samsara 向死而驗 — every UI surface should make silent failure visible, not hide it under decoration. Honesty over polish.
- **Why Claude over Linear/PostHog:** SecondSight observes Claude-family agents; adopting Claude's visual language signals "same ecosystem, same care." Linear gives us the dashboard discipline; PostHog's hedgehog playfulness conflicts with the Samsara seriousness.

### Surface modes (SecondSight diverges)
Claude alternates `canvas → cream-card → dark-mockup` band-by-band as page rhythm. **SecondSight is a single application surface, not a marketing scroll** — there are no "bands." Instead:

- **App background** (`--background: 42 40% 96%` ≈ #f8f4ec) stays near `{colors.canvas}` (#faf9f5). Slightly warmer + slightly darker — reads better behind translucent header tiles.
- **Ambient gradient wash** is added on `<body>` (orange radial top-left, teal radial top-right, soft white-to-cream linear). Claude has nothing like this; it's intentional warmth for a dashboard that users sit in for long sessions. **Don't intensify it** — the gradients are barely-perceptible atmosphere, not decoration.
- **Glassmorphic header tile** (`bg-white/55 + backdrop-blur-xl`) sits as the fixed project-switcher + nav strip. This is a SecondSight invention — Claude doesn't use translucency anywhere. Justification: the surface needs to feel "above" the canvas without printing a hard rectangle through dense data below.
- **No dark navy product surfaces** by default. Claude uses `{colors.surface-dark}` for code-window cards in marketing; SecondSight is *already* the product, so we don't show "the product chrome at scale" as decoration. Reserve `{colors.surface-dark}` only for: (a) real code/diff blocks inside detail panels, (b) the future "directive source trace" terminal view.

### Color overrides (SecondSight palette)
```
--background:  42 40% 96%   /* warm cream — replaces {colors.canvas}  */
--foreground:  214 46% 14%  /* dark navy ink — cooler than {colors.ink} (#141413), suits data-dense reading */
--card:        0 0% 100%    /* pure white card on cream — SecondSight cards float, Claude cards sink */
--primary:     201 79% 33%  /* deep teal — replaces {colors.primary} (coral) for primary surfaces */
--accent:      18 89% 56%   /* terracotta — equivalent to {colors.primary} (#cc785c family) */
--secondary:   38 70% 90%   /* soft amber band — for muted callouts */
--muted:       40 33% 91%   /* dust-cream — for skeleton states / inactive rows */
--destructive: 4 75% 55%    /* same family as {colors.error} (#c64545) */
--border:      214 26% 84%  /* cool hairline — replaces {colors.hairline} (#e6dfd8) */
--radius:      22px         /* much larger than {rounded.lg} (12px) — see Radius override */
```

**Key swap — coral → teal as `primary`, terracotta retained as `accent`.** Claude uses warm coral as `primary` because everything in marketing pushes toward a single CTA. SecondSight has many primary-weight surfaces (Switch project, Enter dashboard, active nav pill, directive-active state). If all of them were coral, the surface would burn. Teal (`201 79% 33%`) carries the primary action role; terracotta (`18 89% 56%`) is reserved for `{component.callout-card-coral}` analogue moments — destructive-or-energetic accents like "Force re-run", "New directive", flagged behavior highlights.

**Cool hairlines + cool ink.** Claude's hairline and ink are warm; SecondSight uses cooler 214° hue. Reason: rows of timestamps, hashes, and IDs read more legibly with a cool axis — warm ink on cream reduces glyph contrast at small sizes.

### Typography overrides
- **No serif display.** Claude's Copernicus is non-negotiable for marketing; SecondSight is a dashboard — serif at small sizes reads as decorative noise on data rows. **Replace with Space Grotesk** (humanist geometric sans) for all display + UI. This is the single largest deliberate divergence.
- **Body / UI:** Space Grotesk replaces both `Copernicus, Tiempos Headline` (display) and `StyreneB, Inter` (body). One family, weight-graded.
- **Mono:** IBM Plex Mono replaces JetBrains Mono. Reason: Plex Mono has better numeric width consistency for the tabular data we render (timestamps, durations, flag IDs); JetBrains is tuned for code prose.
- **Uppercase mono caption (`font-mono uppercase tracking-[0.22em]`)** is the SecondSight signature voice — used on breadcrumb labels ("SecondSight / project-id"), section eyebrows, and load-state captions. Closest Claude analog is `{typography.caption-uppercase}` but at heavier tracking and *in mono*.
- **Tabular numerics.** Always apply `.numeric { font-variant-numeric: tabular-nums; }` to any timestamp / duration / count column. This is non-negotiable. Data must align vertically.

#### Typography voice — label vs value (non-negotiable)

The uppercase + tracking voice above is **LABEL voice**. It must never wrap a raw data value. Two distinct voices coexist:

| Voice | Classes | Use for |
|---|---|---|
| **LABEL** | `font-mono uppercase tracking-[0.22em]` (or `.eyebrow` class) | Section eyebrows ("LEVEL 1", "REASON"), field captions ("Identity", "Frequency"), status enum names ("ACTIVE", "PENDING"), call-to-action labels ("Compatible with") |
| **VALUE** | `font-mono` (just mono, no uppercase, no tracking) — or plain sans for non-technical values | UUIDs, hash IDs, file paths, `identity_key` content, event_id, timestamp content, code fragments |

**Why this matters:** UUIDs forced uppercase (`6E8D0C46-BC93-48E1-BB95-CDF831099133`) lose all visual structure — every char has same ascender height, no x-height variation, eyes can't pattern-match. Lowercase mono (`6e8d0c46-bc93-48e1-bb95-cdf831099133`) is dramatically more readable. Mock data with non-UUID shapes (e.g. `ses_2026-05-14T03:14`) can hide this — *test design against the real data shape*.

**When label + value share a chip** (e.g. `SESSION 6e8d0c46-...` chip), nest the voices: parent applies label voice, child override with `normal-case tracking-normal`:
```html
<div class="font-mono text-xs uppercase tracking-[0.22em] text-primary">
  <span class="opacity-60">Session</span>
  <span class="normal-case tracking-normal">{full_uuid}</span>
</div>
```

**Existing primitive pitfall:** the React `<Badge>` primitive applies LABEL voice to its content unconditionally. When `<Badge>` content is a value (e.g. `truncateMiddle(event.sub_agent_id, 18)` or `truncateMiddle(eventId, 18)`), the value gets force-uppercased — a known bug. Either avoid `<Badge>` for value content, or split it into `<MonoChip>` (label) + `<Chip>` (value) variants. See backlog discussion in [[project_frontend_visual_direction]].

### Radius override (SecondSight cards float bigger)
- Cards: **22px** (`{rounded.lg}` equivalent) — Claude uses 12px. The larger radius is paired with translucent surfaces to read as "floating tiles" rather than "documents."
- Header tile: **32px** outer radius.
- Pill nav / chips: **9999px** (matches Claude `{rounded.pill}`).
- Inputs / buttons: **18px** (= `radius - 4`) — softer than Claude's 8px.
- **Don't migrate to Claude's 8/12/16 scale.** The big-radius look is the SecondSight visual signature; it differentiates from Linear's sharp 6px corners and from generic admin templates.

### Elevation override (Linear-style hairlines for data, ambient shadow for floating tiles)
Claude is **color-block first, shadow rare**. SecondSight needs *two* depth modes:

| Mode | Treatment | Use |
|---|---|---|
| Floating tile | `shadow-ambient` (`0 20px 60px rgba(15,23,42,0.12), 0 6px 18px rgba(15,23,42,0.08)`) + `bg-white/55 backdrop-blur-xl` | Header tile, hero cards, modal sheets |
| Sunken card | Solid `--card` (white) + 1px `--border` hairline, no shadow | Data tables, behavior-flag list rows, directive list |
| Inline divider | 1px `--border` (cooler than Claude hairline) | Inside table rows, between data sections |

The discipline borrowed from **Linear**: hairlines do most of the structural work in data zones — they're cheap, they don't fight the eye on a 100-row list. Reserve the ambient shadow for surfaces that genuinely float above the canvas.

### Component overrides

**`nav-pill`** *(SecondSight-specific — no Claude analog)* — Pill-shaped nav item in the header. Inactive: `bg-white/60` + hairline border + ink text + icon. Active: `bg-primary` (teal) + `text-primary-foreground` (white) + soft shadow. Replaces both Claude's `{component.top-nav}` link row and `{component.category-tab}` paradigm. Reason: SecondSight nav doubles as the *current section indicator on a single-app surface*, not a hover-menu strip.

**`data-table-row`** *(SecondSight-specific)* — `bg-card` (white) + `border-b border-border`. Hovering an entire row brightens it to `bg-muted/40`. No shadow, no radius on the row itself — only the outer table card has 22px radius. This is the workhorse of Observation and Directives views; it gets used hundreds of times per page.

**`behavior-flag-chip`** *(SecondSight-specific)* — Pill badge for behavior flag taxonomy (per SD §5.5.1 vocabulary, with the new `confidence` field). Background scales by severity: low → `bg-muted`, medium → `bg-secondary`, high → `bg-accent/15 text-accent`, critical → `bg-destructive text-destructive-foreground`. Sits in 4–8px gap rows. Replaces Claude `{component.badge-pill}` for any taxonomy display.

**`directive-status-badge`** *(SecondSight-specific)* — Mono caption-uppercase status indicator for directive lifecycle (see `[[project_directive_lifecycle_contract]]` — `pending / active / soft_disabled / archived`). Pure typography + dot, no surface. Reason: status appears 100x per page, must not be visually heavy.

**`button-primary` (override)** — Replace coral with teal. Background `hsl(var(--primary))` (`201 79% 33%`), text `hsl(var(--primary-foreground))` (white), radius 18px, padding 12 × 20. **Active state darkens to `hsl(201 79% 26%)`**. The signature CTA in SecondSight is "Enter dashboard" / "Switch project" / "Apply directive."

**`button-accent`** *(SecondSight-specific)* — Terracotta button for destructive-or-energetic actions: "Soft-disable directive", "Force re-analyze", "Flag for review". Background `hsl(var(--accent))` (terracotta). **Use scarcely** — one accent button per visible panel maximum.

**`callout-card-coral` → `callout-card-terracotta` (renamed)** — Same idea (full-bleed warm callout), but rendered in terracotta accent rather than as the primary action. Used for "no data yet" empty states and the Landing hero panel where the user first lands.

**`code-window-card` (retained from Claude)** — When the Directives detail view shows a directive's source-trace (the actual prompt fragment, commit ref, or log excerpt), render it in the Claude dark-surface code-window aesthetic. This is the *one place* we keep `{colors.surface-dark}` deliberately — it signals "this is the raw artifact, not a rendered summary."

### Layout overrides
- **No 96px section padding.** Dashboard is dense; section padding is `{spacing.lg}` (24px) between major panels, `{spacing.md}` (16px) inside cards.
- **Max content width: 1920px**, not 1200px. SecondSight users have data on wide monitors; we shouldn't pillarbox.
- **Three-pane lookup pattern.** Observation view = list (left) + detail (right). Analysis view = stat strip (top) + flag clusters (grid). Directives view = lifecycle list + edit panel. These are dashboard idioms Claude marketing doesn't cover — derive cards from `feature-card` shape but with `--card` (white) background, hairline border, no shadow.

### Do's and Don'ts (SecondSight-specific, supplements Claude)

#### Do
- Pair teal primary with terracotta accent. Treat them as a **two-key palette** — every visible surface should be answerable to one of them.
- Use uppercase mono captions (`tracking-[0.22em]`) for breadcrumbs, eyebrows, and load states. It's the SecondSight voice.
- Apply `.numeric` (tabular-nums) to every timestamp, hash, count, percentage. Without it data tables jitter.
- Use ambient shadow + translucency for floating tiles; use hairlines for sunken data tables. Don't mix.
- Show the actual product chrome (real prompts, real diffs, real logs) when displaying directive source-trace — same principle as Claude's `{component.code-window-card}`. **No decorative mockups of data we don't have.**
- Reveal silent failure honestly. If polling fails, say "polling stalled at 12:04:33" — don't fake-render the last successful state with no indicator. (Samsara 向死而驗.)

#### Don't
- Don't make the primary CTA terracotta. Terracotta is the **second key**, used scarcely. Teal is primary.
- Don't introduce a third surface family (no green sections, no purple cards, no neon). The cream + teal + terracotta + cool-hairline tetrad is closed.
- Don't add serif headlines. We chose Space Grotesk over Copernicus deliberately for data legibility — don't drift back.
- Don't apply 22px radius to inline elements (rows, chips, table cells). Big radius is a *card-level* signature; on small elements it reads as bubbly.
- Don't fake elevation with both shadow + border on the same surface. Pick one: floating (shadow + translucent) or sunken (hairline + opaque).
- Don't show empty states as "Coming soon." Show what the empty state *means*: "No observation in the last 5 s — last poll at 12:04:33." Empty is information, not failure.
- Don't hide async-fail. If the dashboard polls and 5 s elapses with no response, surface it on the surface that depends on the data — don't swallow into a toast.

### Iteration Guide (SecondSight)
1. Read the Claude sections above first to understand the base atmosphere. Then apply these overrides.
2. When in doubt about which key (teal vs terracotta) a new control belongs to: ask "is this a navigation/state action (teal) or an irreversible/energetic action (terracotta)?"
3. New components default to: `bg-card` white surface, 22px radius if floating / 0px if a list row, hairline border in `--border`, Space Grotesk type, ambient shadow only if it floats.
4. Don't migrate radius scale down to Claude's 8/12/16 — see Radius override.
5. Don't migrate font to serif — see Typography override.
6. If a data-density requirement collides with a Claude rule (e.g. "96px section padding"), prefer the SecondSight rule. Density beats editorial rhythm in dashboard context.

### Known Gaps (SecondSight)
- **Dark mode** — not yet defined. Current cream-canvas direction is light-only. When dark mode is needed (likely from user request), the cooler hairline + cooler ink already half-bridge it; the missing piece is the dark-canvas tone (probably `{colors.surface-dark}` family from Claude).
- **Behavior-flag taxonomy visualization** — chip color scaling above is provisional; once SD §5.5.1 vocabulary + `confidence` field land in [[project_behaviorflag_schema_contract]], revise the severity mapping to match the actual confidence enum.
- **Directive lifecycle visual** — current spec covers status badge only. The lifecycle *transition* affordance (`pending → active → soft_disabled`) needs a dedicated component per `[[project_directive_lifecycle_contract]]`.
- **Empty-state catalog** — listed as a principle ("empty is information"), but a per-view empty-state inventory isn't drawn yet.
- **Polling-stall indicator** — referenced in Do/Don't, but the visual treatment (where it lives, how it surfaces) isn't yet specified.

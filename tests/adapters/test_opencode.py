"""Death tests + unit tests for OpenCodeAdapter (P3B-9, GUR-109).

Death tests verify the silent-failure surface:
    DT-1  supports("nonexistent", *)            -> False
    DT-2  supports("opencode", "<unknown>")      -> False
...
from secondsight.adapters.opencode import (
    DROP_LIST,
    OpenCodeAdapter,
)
...
def test_dt1_unknown_agent_returns_false() -> None:
    ...
    assert adapter.supports("OpenCode", EventType.TOOL_USE_END.value) is False
...
def test_dt4_envelope_missing_session_id_raises() -> None:
    envelope = HookEnvelope.model_construct(
        ...
        timestamp=datetime(2026, 5, 10, 12, 0, tzinfo=timezone.utc),
        sequence_number=0,
        payload={"hook_event_name": "session.created"},
    )
    adapter = OpenCodeAdapter()
...
def test_dt9_data_builders_keys_match_hook_event_types() -> None:
    from secondsight.adapters.opencode import _DATA_BUILDERS, _HOOK_TO_EVENT_TYPE
...
def test_normalize_envelope_fields_forwarded() -> None:
    fixture = _load_fixture(FIXTURE_DIR / "session_created.json")
    envelope = _envelope_from_fixture(fixture)
    adapter = OpenCodeAdapter()
...
def test_tool_execute_after_output_size() -> None:
    fixture = _load_fixture(FIXTURE_DIR / "tool_execute_after.json")
    envelope = _envelope_from_fixture(fixture)
    adapter = OpenCodeAdapter()
    partial = adapter.normalize(envelope, EventType.TOOL_USE_END.value)
    assert partial.data["output_size"] == len(PRIVACY_CANARY_VALUE)
    assert partial.data["success"] is True
...
def test_hook_event_name_mismatch_raises() -> None:
    envelope = HookEnvelope(
        ...
        timestamp=datetime(2026, 5, 10, 12, 0, tzinfo=timezone.utc),
        sequence_number=0,
        payload={"hook_event_name": "session.idle"},
    )
    adapter = OpenCodeAdapter()
    with pytest.raises(ValueError) as exc_info:
        adapter.normalize(envelope, EventType.SESSION_START.value)
    msg = str(exc_info.value)
    assert "session.idle" in msg or "session.created" in msg
...
def test_tool_execute_before_missing_tool_raises() -> None:
    envelope = HookEnvelope(
        project_id="test-proj",
        session_id="sess-1",
        agent=_AGENT_NAME,
